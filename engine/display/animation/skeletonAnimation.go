...
func (this *SkeletonAnimationChannel) SkeletonAnimationChannel(cha *resource.AnimationChannel) {
...
func (this *SkeletonAnimationClip) SkeletonAnimationClip(clip *resource.AnimationClipResource) {
	if clip != nil {
		this.ID = clip.ID
...
func (this *SkeletonAnimationClip) Play() {
	this.playing = true
	this.startTime = int32(time.Now().UnixNano() / 100000)
}
...
func (this *SkeletonAnimationClip) Update() {
	if this.playing {
		this.currentTime = int32(time.Now().UnixNano()/100000) - this.startTime
...
type SkeletonAnimation struct {
	...
	animationClipL map[string]*SkeletonAnimationClip
	currentClip    *SkeletonAnimationClip
}

func (this *SkeletonAnimation) SkeletonAnimation() {
	this.animationClipL = make(map[string]*SkeletonAnimationClip)
}
...
func (this *SkeletonAnimation) AddClip(clip *SkeletonAnimationClip) {
	this.animationClipL[clip.ID] = clip
}
func (this *SkeletonAnimation) SwitchClip(id string, interval int32) bool {
	if clip, ok := this.animationClipL[id]; ok {
...
func (this *SkeletonAnimation) Apply() {
