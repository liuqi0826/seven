...
import (
	"github.com/liuqi0826/seven/engine/display/animation"
...
type Joint struct {
	...
	channel *animation.SkeletonAnimationChannel
}
...
func (this *Joint) Binding(channelList []*animation.SkeletonAnimationChannel) {
	this.channel = channelList[this.Index]
	for _, ch := range this.Children {
		ch.Binding(channelList)
	}
}
...
func (this *Pose) Pose() {
	this.NodeList = make([]*geom.Matrix4x4, 0)
}
...
type Skeleton struct {
	...
	jointTree             *Joint
	pose                  *Pose
	skeletonAnimationClip *animation.SkeletonAnimationClip
}

func (this *Skeleton) Skeleton(skeleton *resource.SkeletonResource) {
	if skeleton != nil {
